...
class Application @Inject() (val env: AuthenticationEnvironment, val messagesApi: MessagesApi) extends AuthenticationController with I18nSupport with DataRequestTable with HasDatabaseConfig[JdbcProfile] {
  ...
  def request(rid: Int) = SecuredAction.async { implicit request =>
    val q = for {
      ...
      u <- r.user
    } yield(r.id, r.title, u.firstName, u.lastName)
